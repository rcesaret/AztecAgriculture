...
AGCensus1960_T_dens = density(AGCensus1960_T$Maize1960_kgha, na.rm = T)
...
AGCensus1991_dens = density(AGCensus1991_T$Yield, na.rm = T)
...
AGCensus2007_dens = density(AGCensus2007_T$Yield, na.rm = T)
...
```{r}
#SIAP_4_2003 = SIAP_4 %>% filter(Year == 2003) %>% group_by(EstadoMunicipio) %>% summarise(AvgYield2003 = sum(Product, na.rm=T)/sum(Harvested, na.rm=T))
#SIAP_4_2003 = density(SIAP_4_2003$AvgYield2003)

SIAP_4_2003a = SIAP_4 %>% filter(Year == 2003) 
SIAP_4_2003 = density(SIAP_4_2003a$Yield)
...
#SIAP_4_2021 = SIAP_4 %>% filter(Year == 2021) %>% group_by(EstadoMunicipio) %>% summarise(AvgYield2021 = sum(Product, na.rm=T)/sum(Harvested, na.rm=T))
#SIAP_4_2021 = density(SIAP_4_2021$AvgYield2021)

SIAP_4_2020a = SIAP_4 %>% filter(Year == 2020) 
SIAP_4_2020 = density(SIAP_4_2020a$Yield)

SIAP_4_2020 <- data.frame(SIAP_4_2020$x, SIAP_4_2020$y, SIAP_4_2020$bw, SIAP_4_2020$n, "SIAP 2020 C. Mexico")
names(SIAP_4_2020) <- c("Yield", "Density", "Bandwidth", "n", "Series")


#SIAP_4_Avg = SIAP_4 %>% group_by(EstadoMunicipio) %>% summarise(AvgYield = sum(Product, na.rm=T)/sum(Harvested, na.rm=T), AvgHarvested = mean(Harvested, na.rm=T))
...
SIAP_4_Avg1 = SIAP_4 %>% group_by(EstadoMunicipio, AGType) %>% summarise(AvgYield = mean(Yield, na.rm=T))

SIAP_4_Avg = density(SIAP_4_Avg1$AvgYield, na.rm=T)
...
SIAP_4_2020$Height = 1100
AGCensus2007_dens$Height = 1100
SIAP_4_2003$Height = 1100
...
Dens.df = rbind(EarlyColonial_dens,LateColonial_dens, SandersAztec_dens, AGCensus1960_T_dens, SandersC20_dens, AGCensus1991_dens, SIAP_4_2003, AGCensus2007_dens, SIAP_4_2020, SIAP_4_Avg)#AGCensus2007_dens

Dens.df$Series <- factor(Dens.df$Series, levels = c("C16 Sample", "C18 Sample", "Sanders Model, Aztec BOM", "1960 AG Census, C. Mexico", "Sanders mid-20th Century BOM", "1991 AG Census, C. Mexico", "SIAP 2003 C. Mexico", "2007 AG Census, C. Mexico", "SIAP 2020 C. Mexico", "SIAP C. Mexico 2003-2021 Average"))
...
ggp1 <- ggplot() + 
  ...
  #scale_y_discrete(expand = c(.05, .05)) +
    scale_x_continuous(limits= c(0,11000)) +#,breaks = MyBreaks,labels = MyLabs
