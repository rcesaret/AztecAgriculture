...
writeRaster(slope,paste0(wd$data_p,"temp_rasts/Slope_15m.tif"), overwrite=TRUE)
wbt_breach_depressions(dem = paste0(wd$data_r,"DEM.tif"), output = paste0(wd$data_p,"temp_rasts/DEM_breach_15m.tif"))
wbt_d8_flow_accumulation(paste0(wd$data_p,"temp_rasts/DEM_breach_15m.tif"), output=paste0(wd$data_p,"temp_rasts/Accum_15m.tif"), out_type = "specific contributing area")
Accum <- rast(paste0(wd$data_p, "temp_rasts/Accum_15m.tif"))
wbt_wetness_index(sca=paste0(wd$data_p,"temp_rasts/Accum_15m.tif"), slope=paste0(wd$data_p,"temp_rasts/Slope_15m.tif"), output=paste0(wd$data_p,"temp_rasts/TWI_15m.tif"), verbose_mode = FALSE)
TWI <- rast(paste0(wd$data_p, "temp_rasts/TWI_15m.tif"))
TWI <- focal(TWI, w=17, fun='max', na.policy="only", na.rm=T)
wbt_stream_power_index(sca=paste0(wd$data_p,"temp_rasts/Accum_15m.tif"),slope=paste0(wd$data_p,"temp_rasts/Slope_15m.tif"),output=paste0(wd$data_p,"temp_rasts/SPI_15m.tif"),exponent = 1)
SPI <- rast(paste0(wd$data_p, "temp_rasts/SPI_15m.tif"))
wbt_fd8_flow_accumulation(paste0(wd$data_p,"temp_rasts/DEM_breach_15m.tif"), output=paste0(wd$data_p,"temp_rasts/Accum_fd8_15m.tif"), out_type = "specific contributing area")
Accum_fd8 <- rast(paste0(wd$data_p, "temp_rasts/Accum_fd8_15m.tif"))
wbt_sediment_transport_index(sca=paste0(wd$data_p,"temp_rasts/Accum_fd8_15m.tif"),slope=paste0(wd$data_p,"temp_rasts/Slope_15m.tif"),output=paste0(wd$data_p,"temp_rasts/STI_15m.tif"))
STI <- rast(paste0(wd$data_p, "temp_rasts/STI_15m.tif"))

Topo_r_15m <- (c(DEM, slope, Accum, Accum_fd8, TRI, TWI, STI, SPI))

nam = c("DEM", "Slope", "Accum", "Accum_fd8", "TRI", "TWI", "STI", "SPI")

names(Topo_r_15m) = nam

rm(slope, Accum, Accum_fd8, TRI, TWI, STI, SPI, nam)

writeRaster(Topo_r_15m, filename = paste0(wd$data_p, "Topo_r_15m.tif"), overwrite=TRUE)
...
CONABIO_2015_LandUse_15m <- resample(CONABIO_2015_LandUse_30m, DEM, method = "near")
CONABIO_2015_LandUse_15m <- crop(CONABIO_2015_LandUse_15m, DEM)
...
q = ifel(CONABIO_2015_LandUse_15m == 18 | CONABIO_2015_LandUse_15m == 19, 1, 0)
Sampling_Area <- q + Sampling_Area 
#qq <- q + qq 

#All geomorphons == 2
q = Geomorphons * Sampling_Area
q = ifel(Geomorphons == 2, 1, 0)
Sampling_Area <- q + Sampling_Area 
#qq <- q + qq 
...
q = ifel(CONABIO_2015_LandUse_15m < 15 | CONABIO_2015_LandUse_15m == 16, 1, 0)
Sampling_Area2 <- q + Sampling_Area2

#Sampled geomorphons == 9,10 with higher accum
q = ifel(Geomorphons == 9 | Geomorphons == 10 , 1, 0)
Sampling_Area2 <- q + Sampling_Area2

#Sampled BDRICM < 100
q = ifel(Soil_15m[["BDRICM"]] < 100, 1, 0)
Sampling_Area2 <- q + Sampling_Area2

#Sampled BDRLOG > 40
q = ifel(Soil_15m[["BDRLOG"]] > 40, 1, 0)
Sampling_Area2 <- q + Sampling_Area2
...
#Sampled Slopes > 15
q = ifel(Topo_r_15m[["Slope"]] > 15, 1, 0)
Sampling_Area2 <- q + Sampling_Area2

#STI >15*********
q = ifel(Topo_r_15m[["STI"]] > 15, 1, 0)
Sampling_Area2 <- q + Sampling_Area2
...
s = spatSample(Sampling_Area2, size = 200000, method="stratified", replace=FALSE, na.rm=T, exhaustive=T)

rc <- c(3100, 10000, 1,  0.25, 1.5, NA)
rc <- matrix(rc, ncol=3, byrow=TRUE)
Sampling_Area <- classify(SIAP_2015_Fields_r, rc)

z = spatSample(Sampling_Area, size = 200000, method="stratified", replace=FALSE, na.rm=T, exhaustive=T)
, as.points=T
z <- as(z, "Spatial")
v <- terra::extract(Sampling_Area, z, cells=T)
v <- unique(v[,3])
m = Sampling_Area
Sampling_Area[v] <- 2
...
v <- extract(Sampling_Area, z, cellnumbers=T)

df = crds(SIAP_2015_Fields_r, df=T, na.rm=T)
...
m_df = Terra_df(SIAP_2015_Mask, "Train_States_Mask")
names(m_df) <- c("x", "y", "Train_States_Mask", "outside")
r_df = Terra_df(r, "Arable")
names(m_df) <- c("x", "y", "Arable", "can_sample")
r_df = r_df %>% 
...
m <- vect(paste0(wd$data_p, "Municipios2000s_Data.gpkg"))
Prediction_mask <- rasterize(SIAP_2015_Fields, DEM, values = NA, background = 1)
Prediction_mask <- mask(Prediction_mask, m)
...
# Define your reclassification rules
reclass_rules <- c(0:50, 1,
                  51:100, 2,
                  101:150, 3)

# Reclassify the raster using the defined rules
reclassified_raster <- reclassify(r, reclass_rules)
m <- c(-101, -98, 1,  0.25, 0.5, 2,  0.5, 1, 3)
rclmat <- matrix(m, ncol=3, byrow=TRUE)
rc <- reclassify(SIAP_2015_Fields_r, rclmat, include.lowest=TRUE)
