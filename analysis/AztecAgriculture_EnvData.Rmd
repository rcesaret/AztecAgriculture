...
fcf <- project(fcf, t, snap = "out")
names(fcf) <- "fcf"
fcf_r <- resample(fcf_r, resample_100m, method = "bilinear")

# PDSI
pdsi_r = pdsi(pr_r[1], pet_r[1])
for (i in 1:12) {
  temp = pdsi(pr_r[i], pet_r[i])
  q = ifelse(i < 10, "0", "")
