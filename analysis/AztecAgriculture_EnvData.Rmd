...
DEM <- ifel(DEM == 0 | is.nan(DEM), NA, DEM)
...
writeRaster(DEM,paste0(wd$data_p,"DEM_15m.tif"), overwrite=TRUE)
#DEM <- rast(paste0(wd$data_p,"DEM_15m.tif"))

DEM_100m <- resample(DEM, resample_100m, method = "bilinear")
...
wbt_breach_depressions(dem = paste0(wd$data_r,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/DEM_breach.tif"))
...
wbt_d_inf_pointer(dem = paste0(wd$data_p,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/FlowDirect_inf_100m.tif"))
FlowDirect_inf_30m <- rast(paste0(wd$data_p, "temp_rasts/FlowDirect_inf_100m.tif"))

wbt_d8_pointer(dem = paste0(wd$data_p,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/FlowDirect_d8_100m.tif"))
...
BP = extract(DEM_100m, BasinsPoly, fun=ncell)
BasinsPoly$n_cells = BP$focal_mean
rm(BP)
bp <- terra::rasterize(BasinsPoly, DEM_100m, field = "n_cells", background = NA)
...
wbt_elevation_above_stream(dem = paste0(wd$data_p,"DEM_100m.tif"), streams = paste0(wd$data_p,"temp_rasts/Streams_100m.tif"), output = paste0(wd$data_p,"temp_rasts/ElevAboveStreams_100m.tif"))
ElevAboveStreams <- rast(paste0(wd$data_p, "temp_rasts/ElevAboveStreams_100m.tif"))

######################################################

wbt_elevation_above_stream(dem = paste0(wd$data_p,"DEM_100m.tif"), streams = paste0(wd$data_p,"Streams2_100m.tif"), output = paste0(wd$data_p,"ElevAboveStreams2_100m.tif"))
ElevAboveStreams2 <- rast(paste0(wd$data_p, "ElevAboveStreams2_100m.tif"))

######################################################

wbt_extract_valleys(dem = paste0(wd$data_p,"DEM_100m.tif"), variant = 'JandR', line_thin = F, output =  paste0(wd$data_p,"temp_rasts/valleys_JandR_100m.tif"))
...
Basins <- terra::rasterize(BasinsPoly, DEM_100m, field = "ID", background = NA)
writeRaster(Basins,paste0(wd$data_p,"Basins_100m.tif"), overwrite=TRUE)
Basins <- rast(paste0(wd$data_p, "Basins_100m.tif"))

######################################################

wbt_elev_relative_to_watershed_min_max(dem = paste0(wd$data_p,"DEM_100m.tif"), watersheds = paste0(wd$data_p,"Basins_100m.tif"), output = paste0(wd$data_p,"temp_rasts/elev_watershed_100m.tif"))
elev_watershed <- rast(paste0(wd$data_p, "temp_rasts/elev_watershed_100m.tif"))

wbt_minimal_curvature(dem = paste0(wd$data_p,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_min_100m.tif"))
curv_min <- rast(paste0(wd$data_p, "temp_rasts/curv_min_100m.tif"))

wbt_maximal_curvature(dem = paste0(wd$data_p,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_max_100m.tif"))
curv_max <- rast(paste0(wd$data_p, "temp_rasts/curv_max_100m.tif"))

wbt_mean_curvature(dem = paste0(wd$data_p,"DEM_100m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_mean_100m.tif"))
curv_mean <- rast(paste0(wd$data_p, "temp_rasts/curv_mean_100m.tif"))


```

Topo_r_30m = c(Topo_r_30m, )#, , 
names(Topo_r_30m) = c(names(Topo_r_30m), "DistStreams", "DistStreams2", "ElevAboveStreams", "ElevAboveStreams2", "", "elev_watershed", "FloodOrder_Rel", "", "", "")
...
rm(AWCtS_0,AWCtS_5,AWCtS_15,AWCtS_30,AWCtS_60,AWCtS_100,AWCtS_200, AWCtS_2mi, AWCtSi)
...
rm(Zi, Si, CFi, SOCi, CECi, OCDi, Ni, SOCSi, Ci, BDi, AWCtS_2mi, AWCtSi, BDTICMi, BDRLOGi, BDRICMi)
```
...
#Stack Rasters
Soil = c(BDRICM, BDRLOG, BDTICM, AWCtS, AWCtS_2m, BD, S, Z, C, CF, SOC, CEC, OCD, N, SOCS, WRB)

# Export Soil Raster Stack
writeRaster(Soil, filename = paste0(wd$data_p, "Soil_r_resampled.tif"), overwrite=TRUE)

#Remove unnecessary variables
rm(BDRICM, BDRLOG, BDTICM, AWCtS, AWCtS_2m, BD, S, Z, C, CF, SOC, CEC, OCD, N, SOCS, WRB)
...
rm(bb,ulx, uly, lrx, lry, files, URLs, month, year, var, var2, o, EXT, nom, bbox, chwd)
...
# states that monthly pr units are kg m-2 month-1 /100
# may need to multiply pr by 100 
