...
Topo_r_30m <- c(DEM, slope, Accum_fd8, TRI, TWI, STI, SPI, flood_order, valleys_lq, elev_watershed, curv_vert_excess, curv_horiz_excess, curv_min, curv_max, curv_mean)

names(Topo_r_30m) = c("DEM", "Slope", "Accum_fd8", "TRI", "TWI", "STI", "SPI", "flood_order", "valleys_lq", "elev_watershed", "curv_vert_excess", "curv_horiz_excess", "curv_min", "curv_max", "curv_mean")
...
levels(Geomorphons) = as.data.frame(levels(Geomorphons)[[1]]) %>% select(-CMex_15m_Geomorphons) %>%
...
CMex_Mask <- st_read(paste0(wd$data_p, "Municipios2000s_Data.gpkg"))
...
SIAP_LU_Fields <- st_read(paste0(wd$data_p, "Data000s_poly.gpkg"))
...
SIAP_LU_Fields_AGType <- rasterize(SIAP_LU_Fields_AGType, DEM, values = "AGType", background = NA)
...
rm(SIAP_LU_Fields_AGType, streams)
...
if (dat == "SIAP_2015_Fields") {
  ...
  x = c(Cultiv, CONABIO_2015_LandUse_30m, Topo_r_30m, Soil_30m, Geomorphons)
...
fLU = freq(CONABIO_2015_LandUse_30m, usenames=T, zones=Cultiv, wide=T)
...
fgeo = freq(Geomorphons, usenames=T, zones=Cultiv, wide=T)
...
Arable_RF_df <- Arable_RF_df %>% 
  mutate(SIAP_2015_Fields_f = factor(Cultiv, levels = c("Uncultivated", "Cultivated"))) 
#%>%
...
rm(Topo_r_30m, Soil_30m, SIAP_LU_Fields_AGType)
```
...
write_csv(x_df, paste0(wd$data_p,"ArableArea_RFModel_Data.csv.gz"))
rm(x_df)
AGType_RF_df <- read_csv(paste0(wd$data_p,"ArableArea_RFModel_Data.csv.gz"))
...
write_csv(AGType_RF_Train_df, paste0(wd$data_p,"ArableArea_RFModel_TrainData.csv.gz"))
rm(AGType_RF_Train_df)
AGType_RF_Train_df <- read_csv(paste0(wd$data_p,"ArableArea_RFModel_TrainData.csv.gz"))
...
write_csv(AGType_RF_df, paste0(wd$data_p,"ArableArea_RFModel_Data.csv.gz"))
rm(AGType_RF_df)
AGType_RF_df <- read_csv(paste0(wd$data_p,"ArableArea_RFModel_Data.csv.gz"))

AGType_RF_Predict_df = AGType_RF_df %>% filter(PredictionData = T)
  
write_csv(AGType_RF_Predict_df, paste0(wd$data_p,"ArableArea_RFModel_PredictData.csv.gz"))
rm(AGType_RF_Predict_df)
AGType_RF_Predict_df <- read_csv(paste0(wd$data_p,"ArableArea_RFModel_PredictData.csv.gz"))
