...
Arable_RF_Predict_df %>%
  mutate(part = case_when(
    row_number() <= ceiling(nrow(.) / 5) ~ "ArableArea_RFModel_PredictData1",
    row_number() <= 2 * ceiling(nrow(.) / 5) ~ "ArableArea_RFModel_PredictData2",
    row_number() <= 3 * ceiling(nrow(.) / 5) ~ "ArableArea_RFModel_PredictData3",
    row_number() <= 4 * ceiling(nrow(.) / 5) ~ "ArableArea_RFModel_PredictData4",
    TRUE ~ "ArableArea_RFModel_PredictData5"
  )) %>%
  group_split(part) %>%
  purrr::walk2(
    .,
    c("ArableArea_RFModel_PredictData1", "ArableArea_RFModel_PredictData2", "ArableArea_RFModel_PredictData3", "ArableArea_RFModel_PredictData4", "ArableArea_RFModel_PredictData5"),
...
l = 1:5
file_paths <- paste0(wd$data_p, "ArableArea_RFModel_PredictData", l, ".csv.gz")

Arable_RF_Predict_df <- file_paths %>%
  map_dfr(~ read_csv(.x))
...
#Arable_RF_df <- Arable_RF_df %>% select(-)

#Arable_RF_Train_df <- Arable_RF_df %>% select(-)

#Arable_RF_Predict_df <- Arable_RF_df %>% select(-)
...
#loading the model
#RF_model = readRDS(paste0(wd$data_p,"ArableArea_RFModel_Ranger.rda"))
```
...
# saving the model
saveRDS(RF_pred, file = paste0(wd$data_p,"ArableArea_RFModel_PredictObj.rda"))

#loading the model
#RF_pred = readRDS(paste0(wd$data_p,"ArableArea_RFModel_PredictObj.rda"))
