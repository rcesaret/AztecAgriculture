...
Pred_Poly_ModLU <- Data2000s_poly_rs %>%  mutate(
    AvgPctArea_ofTotal = case_when(
      EstadoMunicipio == "Ixtapaluca, Mexico" & AGType == "Riego" ~ 1200,
      EstadoMunicipio == "Valle de Chalco Solidaridad, Mexico" & AGType == "Riego" ~ 1200,
      EstadoMunicipio == "Milpa Alta, Distrito Federal" & AGType == "Riego" ~ 2000,
      EstadoMunicipio == "Tlahuac, Distrito Federal" & AGType == "Riego" ~ 2000,
      EstadoMunicipio == "Xochimilco, Distrito Federal" & AGType == "Riego" ~ 1200,
      EstadoMunicipio == "Axapusco, Mexico" & AGType == "Riego" ~ 1500
...
Pred_Poly_ModMun_Temp = st_drop_geometry(Pred_Poly_ModLU) %>% filter(AGType == "Temp") 
Pred_Poly_ModMun_Temp = Municipios2000s_Data %>% left_join(Pred_Poly_ModMun_Temp, by = c("EstadoMunicipio", "Estado", "Municipio"))

Pred_Poly_ModMun_Irrig = st_drop_geometry(Pred_Poly_ModLU) %>% filter(AGType == "Irrig") 
Pred_Poly_ModMun_Irrig = Municipios2000s_Data %>% left_join(Pred_Poly_ModMun_Irrig, by = c("EstadoMunicipio", "Estado", "Municipio"))
...
levels(AGType_Temp) = levels(AGType_Temp)[[1]] %>% select(-layer) %>%
...
AG_Workers_DensArable_Temp <- terra::rasterize(AG_Workers_DensArable_Temp, Topo[[1]], 
                                               field = "AG_Workers_DensArable", background = NA)
AG_Workers_DensArable_Temp = mask(AG_Workers_DensArable_Temp, ArableAreaMask)

Pop_DensArable_Temp <- terra::rasterize(Pop_DensArable_Temp, Topo[[1]], field = "Pop_DensArable", background = NA)
...
AG_Workers_DensArable_Irrig <- terra::rasterize(AG_Workers_DensArable_Irrig, Topo[[1]], 
                                               field = "AG_Workers_DensArable", background = NA)
AG_Workers_DensArable_Irrig = mask(AG_Workers_DensArable_Irrig, ArableAreaMask)

Pop_DensArable_Irrig <- terra::rasterize(Pop_DensArable_Irrig, Topo[[1]], field = "Pop_DensArable", background = NA)
...
Clim_sub <- mask(Clim_sub, ArableAreaMask)

Clim_df <- as.data.frame(Clim_sub[[1]])
names(Clim_df) <- names(Clim_sub[[1]])
Clim_df <- rownames_to_column(Clim_df, var = "Cells")

for (i in 2:nlyr(Clim_sub)) {
  
  t = as.data.frame(Clim_sub[[i]])
  t <- rownames_to_column(t, var = "Cells")
  names(t) <- c("Cells", names(Clim_sub[[i]]))
  Clim_df <- Clim_df %>% left_join(t, by="Cells")

}

AGRastTemp_df <- (AGRastsRF_Temp[[1]])
...
AGRastIrrig_df <- (AGRastsRF_Irrig[[1]])
...
RF_df_Temp = cbind(AGRastTemp_df, Clim_df) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Irrigation")))
RF_df_Irrig = cbind(AGRastIrrig_df, Clim_df) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Irrigation")))
