...
MilpaAlta <- SIAP_2 %>% filter(EstadoMunicipio == "Milpa Alta, Distrito Federal" & AGType == "Riego" & Cycle == "Perennes") %>% mutate(Cultigen = "Maiz grano", Cultigen_ID = 7490000, AGType = "Riego", Price = NA, Value = NA, Yield_orig = NA, Yield_SIAP = NA, Planted = Planted +20, Harvested = Harvested + 20) 
...
ValleChalco$Yield = (Chalco$ZYield[(length(Chalco$ZYield)-nrow(ValleChalco)):length(Chalco$ZYield)] + runif(1, min = -0.5, max = 0.5))*(150 +runif(1, min = 0, max = 50))+3900
...
Ixtapaluca$Yield = (Chalco$ZYield + runif(1, min = -0.5, max = 0.5))*(200 +runif(1, min = -10, max = 20))+2300
...
Tonanitla$Yield = (Chalco$ZYield + runif(1, min = -0.5, max = 0.5))*(200 +runif(1, min = -10, max = 20))+3800
...
ADDEDCASES <- rbind(MilpaAlta, Xochimilco, Tlahuac, ValleChalco, Ixtapaluca, Axapusco, Nextlalpan, Tonanitla)

rm(Chalco, MilpaAlta, Xochimilco, Tlahuac, ValleChalco, Ixtapaluca, Axapusco, Nextlalpan, Otumba, Tonanitla)
...
write.csv(SIAP_2b, paste0(wd$data_r,"SIAP_2_rescale.csv"))
