...
rsds_summer = app(subset(Clim, c("rsds_07","rsds_08", "rsds_09")), mean)
rsds_sept = subset(Clim, c("rsds_09"))
rsds_oct = subset(Clim, c("rsds_10"))

Clim_sub <- subset(Clim, c("npp", "swb", "gdd10"))

Clim_sub <- c(Clim_sub, pr_may, pr_summer, pr_fall, pet_spring, pet_summer, pet_oct, cmi_may, cmi_summer, cmi_fall, clt_may, clt_summer, clt_fall, rsds_may, rsds_june, rsds_summer, rsds_oct)

names(Clim_sub) <- c("npp", "swb", "gdd10", "pr_may", "pr_summer", "pr_fall", 'pet_spring', "pet_summer", "pet_oct", "cmi_may", "cmi_summer", "cmi_fall", "clt_may", "clt_summer", "clt_fall", "rsds_may", "rsds_june", "rsds_summer", "rsds_oct")

writeRaster(Clim_sub, filename = paste0(wd$data_p, "Clim_sub_RF.tif"), overwrite=TRUE)

rm(Clim_sub, pr_may, pr_summer, pr_fall, pet_spring, pet_summer, pet_oct, cmi_may, cmi_summer, cmi_fall, clt_may, clt_summer, clt_fall, rsds_may, rsds_june, rsds_summer, rsds_oct, Clim, Soil)
...
```{r}
tt=Train_df[,c(4:5,7:ncol(Train_df))]
corm = cor(tt, use="pairwise.complete.obs", method="pearson")
```
...
RasterizePoly = Train_sf %>% select(AGType:PopRural_DensArable)
nn = ncol(RasterizePoly) - 1
vars = names(RasterizePoly)[1:nn]
temp = RasterizePoly %>% select(!!sym(vars[1]))
AG_rasts_train <- terra::rasterize(temp, Env_sub, field = vars[1], background = NA)

for (i in 2:nn){
  temp = RasterizePoly %>% select(!!sym(vars[i]))
  temp_rast <- terra::rasterize(temp, Env_sub, field = vars[i], background = NA)
  AG_rasts_train <- c(AG_rasts_train, temp_rast)
}
...
Train_df_Temp = Train_df %>% filter(AGType == "Temporal")
Train_df_Irrig = Train_df %>% filter(AGType == "Riego")
#par(mar = c(11, 4, 4, 2))

#boruta = readRDS(paste0(wd$data_p,"AGType_RFModel_boruta.rda"))
boruta_Bavg <- Boruta(AvgYield ~ ., data = Train_df[,c(4, 6:ncol(Train_df))], maxRuns = 500)#, doTrace = 2
plot(boruta_Bavg, las = 2, cex.axis = 0.7)
Bavg_atts <- attStats(boruta_Bavg)
Bavg_form = getConfirmedFormula(boruta_Bavg)

boruta_Tavg <- Boruta(AvgYield ~ ., data = Train_df_Temp[,c(4, 6:ncol(Train_df_Temp))], maxRuns = 500)
Tavg_atts <- attStats(boruta_Tavg)

boruta_Iavg <- Boruta(AvgYield ~ ., data = Train_df_Irrig[,c(4,6:ncol(Train_df_Irrig))], maxRuns = 500)
...
AGType, mean.CEC, mean.OCD, mean.ElevAboveStreams, AvgPctArea_ofTotal, AG_Workers_DensArable, mean.C, mean.BDRICM, 
Fertilizer, mean.DEM, mean.rsds_may, mean.rsds_summer, mean.gdd10, mean.pr_fall, mean.DistStreams, mean.Slope, 
mean.N, mean.cmi_summer, mean.pr_summer, TotalLabor_DensArable, mean.npp, mean.swb, mean.pet_spring, mean.TWI, mean.pet_oct, 
mean.cmi_may, mean.pet_summer, mean.rsds_june, mean.rsds_oct

```{r}
#Train_sf_Total = Train_sf %>% select(AvgYield, AGType, mean.CEC, mean.OCD, mean.ElevAboveStreams, AvgPctArea_ofTotal, AG_Workers_DensArable, mean.C, mean.BDRICM, Fertilizer, mean.DEM, mean.rsds_may, mean.rsds_summer, mean.gdd10, mean.pr_fall, mean.DistStreams, mean.Slope, mean.N, mean.cmi_summer, mean.pr_summer, TotalLabor_DensArable, mean.npp, mean.swb, mean.pet_spring, mean.TWI, mean.pet_oct, mean.cmi_may, mean.pet_summer, mean.rsds_june, mean.rsds_oct)
boruta_Bavg <- Boruta(AvgYield ~ ., data = Train_df[,c(4, 6:ncol(Train_df))], maxRuns = 500)#, doTrace = 2
Bavg_atts <- attStats(boruta_Bavg)
Bavg_atts$Boruta <- scales::rescale(rank(Bavg_atts$meanImp)/nrow(Bavg_atts))
Bavg_atts$feature = row.names(Bavg_atts)
FeatSelect = Bavg_atts %>% rowid_to_column(var = "ORD") %>% select(ORD, feature, meanImp, Boruta)

centroids <- st_centroid(Train_sf) %>% select(-Estado, -Municipio, -EstadoMunicipio, -cvYield)

Tsk_Y_sf_Total = as_task_regr_st(centroids, target = "AvgYield", id = "Train_sf_Total")

methodz <- c("jmi", "mrmr", "njmim", "cmim", "disr")

for (i in 1:length(methodz)){
  temp = flt(methodz[i])
  temp$calculate(Tsk_Y_sf_Total)
...
```{r}
Train_sf_Temp_AvgYield = Train_sf %>% filter(AGType == "Temporal")
df = st_drop_geometry(Train_sf_Temp_AvgYield)

boruta_Bavg <- Boruta(AvgYield ~ ., data = df[,c(4, 7:ncol(df))], maxRuns = 500)#, doTrace = 2
...
centroids <- st_centroid(Train_sf_Temp_AvgYield) %>% select(-Estado, -Municipio, -EstadoMunicipio, -cvYield, -AGType)
...
Train_sf_Temp_AvgYield = centroids %>% select(AvgYield, mean.gdd10, mean.rsds_may, mean.OCD, mean.S, mean.npp, mean.swb, mean.pet_spring, mean.CEC, TotalLabor_DensArable, mean.clt_fall, AvgPctArea_ofTotal, mean.CF, mean.pr_fall, mean.cmi_may, mean.pr_summer, mean.clt_may, mean.cmi_summer, Fertilizer, mean.pet_summer, mean.rsds_summer, mean.pet_oct, mean.BDRICM, mean.Z, mean.pr_may, mean.N, mean.clt_summer, mean.rsds_oct, mean.DEM, mean.cmi_fall, mean.FloodOrder_Rel)
```
...
names(Train_sf_Temp_AvgYield) = sub("^mean\\.", "", names(Train_sf_Temp_AvgYield))

# Dataset ("Task")
#Both_AvgYield = as_task_regr(x = Train_df_Both_AvgYield[,c(4:ncol(Train_df_Both_AvgYield))], target="AvgYield", id = "Both_AvgYield")
Temp_AvgYield = as_task_regr_st(Train_sf_Temp_AvgYield, target = "AvgYield", id = "Train_sf_Total")
Temp_AvgYield_train = as_task_regr_st(Train_sf_Temp_AvgYield, target = "AvgYield", id = "Train_sf_Total")
Temp_AvgYield_test = as_task_regr_st(Train_sf_Temp_AvgYield, target = "AvgYield", id = "Train_sf_Total")
# Partition the data with the default 2/3 split (== 2/3 data for training, 1/3 for testing)
splits = partition(Temp_AvgYield, ratio=0.67)
#rcv = rsmp("spcv_coords", folds = 2)
#rcv$instantiate(Both_AvgYield)
#Both_AvgYield_test$filter(rcv$test_set(2))
#Both_AvgYield_train$filter(rcv$train_set(2))
Temp_AvgYield_train = Temp_AvgYield_train$filter(splits$train)

Temp_AvgYield_test = Temp_AvgYield_test$filter(splits$test)
#Both_AvgYield = as_task_regr(x = Train_df_Both_AvgYield[,c(4:ncol(Train_df_Both_AvgYield))], target="AvgYield", id = "BoAvY")
Temp_AvgYield = as_task_regr_st(Train_sf_Temp_AvgYield, target = "AvgYield", id = "Train_sf_Total")
#### Initial Random Forest Model Parameter Tuning
...
# Results of Tuning via grid search:
ToAvY_instance$result$learner_param_vals

insample = ToAvY_instance$result_y
...
rsmp_cv5 = rsmp("cv", folds = 5)
#rcv$instantiate(Both_AvgYield_train)
outsample = resample(Temp_AvgYield_train, ToAvY_at, rsmp_cv5)$aggregate(msr("regr.mae"))

# setup another Ranger Random Forest Regression Model
RegRF_tuned = lrn("regr.ranger")

# Set the tuned parameter values
RegRF_tuned$param_set$values = ToAvY_instance$result_learner_param_vals

generalization = RegRF_tuned$train(Temp_AvgYield_train)$
  predict(Temp_AvgYield_test)$score(msr("regr.mae"))

rbind(generalization,insample,outsample)

###### Benchmarking

tasks = list(Temp_AvgYield)
learners = list(RegRF_tuned, lrn("regr.rpart"), lrn("regr.ranger"), lrn("regr.ksvm"), lrn("regr.bart"), lrn("regr.xgboost"), lrn("regr.svm"))
resamplings = list(rsmp_cv5)
