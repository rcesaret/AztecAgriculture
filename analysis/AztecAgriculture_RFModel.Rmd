...
### PCA for Agricultural Variable Selection
...
pca_tot <- prcomp(Tot[,-c(1:6)], center = TRUE, scale. = TRUE)
loadings_tot = as.data.frame(pca_tot$rotation, row.names = colnames(Tot[,-c(1:6)]))

pca_temp <- prcomp(Temp[,-c(1:5)], center = TRUE, scale. = TRUE)
loadings_temp = as.data.frame(pca_temp$rotation, row.names = colnames(Temp[,-c(1:5)]))

pca_irrig <- prcomp(Irrig[,-c(1:5)], center = TRUE, scale. = TRUE)
loadings_irrig = as.data.frame(pca_irrig$rotation, row.names = colnames(Irrig[,-c(1:5)]))
