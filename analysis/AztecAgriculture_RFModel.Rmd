...
Arable <- Arable %>% mutate(Arable_ha = sum*res(ArableArea_100m)[1]*res(ArableArea_100m)[2]/10000,
                            PctMun_Arable = Arable_ha / Area_ha) %>% 
                     rename(AreaMun_ha = Area_ha) %>% select(-sum)

Data2000s_poly_rs <- Data2000s_poly_rs %>% 
  left_join(Arable, by = c("EstadoMunicipio","Estado", "Municipio"))
  
  
rm(Arable, ArableArea_100, temp)
...
LandUse_100m <- ifel(LandUse_100m == 17, 1, 0)

Settlement <- exact_extract(x = LandUse_100m, y = Municipios2000s, fun = "sum", 
                           max_cells_in_memory = 8e+08, stack_apply=T, 
                           append_cols = c("EstadoMunicipio","Estado", "Municipio"))

Data2000s_poly_rs <- Data2000s_poly_rs %>% 
  left_join(Arable, by = c("EstadoMunicipio","Estado", "Municipio")) %>% 
  mutate(Settlement_ha = sum*res(LandUse_100m)[1]*res(LandUse_100m)[2]/10000,
         PctMun_Settlement = Settlement_ha / AreaMun_ha,
         AvgMunSettlementDens = Pop_Total / Settlement_ha) %>% 
  select(-sum)

rm(Settlement, LandUse_100m, CONABIO_2015_LandUse_30m)
...
Data2000s_poly_rs <- Data2000s_poly_rs %>% mutate(
  ...
  AvgPctArea_Irrig = AvgArea_Irrig / AvgArea_Total,

  # temporal % of total cultivation 
  AvgPctArea_Temp = AvgArea_Temp / AvgArea_Total,

  #AvgPctArea_Maize_Total # == maize % of total cultivated area
  #AvgPctArea_Maize_Crops_Total # == maize % of total crop cultivated area (excl. fodder)

  #AvgPctArea_Maize_Temp # == maize % of total temporal area
  #AvgPctArea_Maize_Crops_Temp # == maize % of total crop temporal area (excl. fodder)

  #AvgPctArea_Maize_Irrig # == maize % of total irrigated area
  #AvgPctArea_Maize_Crops_Irrig # == maize % of total crop irrigated area (excl. fodder)

  # irrigated maize % of total cultivation area
  AvgPctArea_IrrigMaize = AvgArea_Irrig_Maize / AvgArea_Total,

  # temporal maize % of total cultivation area
  AvgPctArea_TempMaize = AvgArea_Temp_Maize / AvgArea_Total)

#########################################################################

# labor and laboresque capital input metrics
...
Data2000s_poly_rs <- Data2000s_poly_rs %>% rowwise() %>% mutate(
  ...
  PopRuralMz = ifelse(AGType == "Temporal", Pop_Rural * AvgPctArea_TempMaize, Pop_Rural * AvgPctArea_IrrigMaize),
  PrimaryMz = ifelse(AGType == "Temporal", Primary * AvgPctArea_TempMaize, Primary * AvgPctArea_IrrigMaize),
  AGWorkersMz = ifelse(AGType == "Temporal", AG_Workers * AvgPctArea_TempMaize, AG_Workers * AvgPctArea_IrrigMaize),
  TotalLaborMz = ifelse(AGType == "Temporal", TotalLabor * AvgPctArea_TempMaize, TotalLabor * AvgPctArea_IrrigMaize),
  LUTotalMz = ifelse(AGType == "Temporal", LU_Total * AvgPctArea_TempMaize, LU_Total * AvgPctArea_IrrigMaize),
  ContractLaboTotMz = ifelse(AGType == "Temporal", ContractLabor_Tot * AvgPctArea_TempMaize, ContractLabor_Tot * AvgPctArea_IrrigMaize),
  ContractLaborFullMz = ifelse(AGType == "Temporal", ContractLabor_More6Mo * AvgPctArea_TempMaize, ContractLabor_More6Mo * AvgPctArea_IrrigMaize),
  ContractLaboPartMz = ifelse(AGType == "Temporal", ContractLabor_Less6Mo * AvgPctArea_TempMaize, ContractLabor_Less6Mo * AvgPctArea_IrrigMaize),
  FamLaborMz = ifelse(AGType == "Temporal", FamLabor_Tot * AvgPctArea_TempMaize, FamLabor_Tot * AvgPctArea_IrrigMaize),
  MechanizEquipMz = ifelse(AGType == "Temporal", MechanizEquip * AvgPctArea_TempMaize, MechanizEquip * AvgPctArea_IrrigMaize),
  DraftAnimalsMz = ifelse(AGType == "Temporal", DraftAnimals_Total * AvgPctArea_TempMaize, DraftAnimals_Total * AvgPctArea_IrrigMaize),
  TractorsMz = ifelse(AGType == "Temporal", Tractors * AvgPctArea_TempMaize, Tractors * AvgPctArea_IrrigMaize),
  ...
  PopRuralMz_perMzHa = ifelse(AGType == "Temporal", PopRuralMz / AvgArea_Temp_Maize, PopRuralMz / AvgArea_Irrig_Maize),
  PrimaryMz_perMzHa = ifelse(AGType == "Temporal", PrimaryMz / AvgArea_Temp_Maize, PrimaryMz / AvgArea_Irrig_Maize),
  AGWorkersMz_perMzHa = ifelse(AGType == "Temporal", AGWorkersMz / AvgArea_Temp_Maize, AGWorkersMz / AvgArea_Irrig_Maize),
  TotalLaborMz_perMzHa = ifelse(AGType == "Temporal", TotalLaborMz / AvgArea_Temp_Maize, TotalLaborMz / AvgArea_Irrig_Maize),
  LUTotalMz_perMzHa = ifelse(AGType == "Temporal", LUTotalMz / AvgArea_Temp_Maize, LUTotalMz / AvgArea_Irrig_Maize),
  ContractLaboTotMz_perMzHa = ifelse(AGType == "Temporal", ContractLaboTotMz / AvgArea_Temp_Maize, ContractLaboTotMz / AvgArea_Irrig_Maize),
  ContractLaborFullMz_perMzHa = ifelse(AGType == "Temporal", ContractLaborFullMz / AvgArea_Temp_Maize, ContractLaborFullMz / AvgArea_Irrig_Maize),
  ContractLaboPartMz_perMzHa = ifelse(AGType == "Temporal", ContractLaboPartMz / AvgArea_Temp_Maize, ContractLaboPartMz / AvgArea_Irrig_Maize),
  FamLaborMz_perMzHa = ifelse(AGType == "Temporal", FamLaborMz / AvgArea_Temp_Maize, FamLaborMz / AvgArea_Irrig_Maize),
  MechanizEquipMz_perMzHa = ifelse(AGType == "Temporal", MechanizEquipMz / AvgArea_Temp_Maize, MechanizEquipMz / AvgArea_Irrig_Maize),
  DraftAnimalsMz_perMzHa = ifelse(AGType == "Temporal", DraftAnimalsMz / AvgArea_Temp_Maize, DraftAnimalsMz / AvgArea_Irrig_Maize),
  TractorsMz_perMzHa = ifelse(AGType == "Temporal", TractorsMz / AvgArea_Temp_Maize, TractorsMz / AvgArea_Irrig_Maize))
...
Train_df = st_drop_geometry(Data2000s_poly_rs) %>% select(EstadoMunicipio, Estado, Municipio, AvgYield, cvYield, AGType, LU_Total_perHa, AvgPctArea_Maize_Total, AvgPctArea_Maize_Crops_Total, PopDens, Pop_Urban, TotalLaborDens, PopDensRural, Pop_Rural, UrbRatio, Pct_Primary, Pct_AG_Workers, Techniques, Fertilizer, AG_Workers_perHa, LU_Total) %>% 
...
Train_df_Temp = x %>% filter(AGType == "Temporal") %>% select(-AGType)

Train_df_Irrig = x %>% filter(AGType == "Riego") %>% select(-AGType)
...
boruta <- Boruta(AvgYield ~ ., data = xxx[,c(4,6:ncol(xxx))], maxRuns = 500)#, doTrace = 2
boruta <- Boruta(AvgYield ~ ., data = xxxT[,c(4,6:ncol(xxxT))], maxRuns = 500)#, doTrace = 2
boruta <- Boruta(AvgYield ~ ., data = xxxI[,c(4,6:ncol(xxxI))], maxRuns = 500)#, doTrace = 2
boruta <- Boruta(cvYield ~ ., data = xxx[,c(5,6:ncol(xxx))], maxRuns = 500)#, doTrace = 2
boruta <- Boruta(cvYield ~ ., data = xxxT[,c(5,6:ncol(xxxT))], maxRuns = 500)#, doTrace = 2
boruta <- Boruta(cvYield ~ ., data = xxxI[,c(5,6:ncol(xxxI))], maxRuns = 500)#, doTrace = 2
print(boruta)
...
RF_model <- ranger(formula = form, data = xxxT)
