...
```{r, label='Calculate Topographic Variable Rasters: 100m', message=FALSE,warning=FALSE}
...
```{r, label='Stack and Save 100m Topographic Variable Rasters', message=FALSE,warning=FALSE}

Topo <- c(DEM_100m, Slope, Slope_sqrt, Accum, Accum_fd8, TRI, TWI, STI, SPI, curv_min, curv_max, curv_mean, DistStreams, DistStreams2, ElevAboveStreams, elev_watershed, FloodOrder_Rel)#ElevAboveStreams2, 

names(Topo) = c("DEM", "Slope", "Slope_sqrt", "Accum", "Accum_fd8", "TRI", "TWI", "STI", "SPI", "curv_min", "curv_max", "curv_mean", "DistStreams", "DistStreams2", "ElevAboveStreams", "elev_watershed", "FloodOrder_Rel")#"ElevAboveStreams2", 

Topo <- project(Topo, resample_100m)
Topo <- resample(Topo, resample_100m, method = "bilinear")
...
rm(DEM_100m, Slope, Slope_sqrt, Accum, Accum_fd8, TRI, TWI, STI, SPI, curv_min, curv_max, curv_mean, DistStreams, DistStreams2, ElevAboveStreams, Isobasins, IsobasinsPoly, FlowDirect_d8_100m, FlowDirect_inf_100m, elev_watershed, FloodOrder_Rel, FloodOrder, Valleys,  Streams, Streams2, Basins, streams_sf, DEM_smooth_100m, DEM)
...
DEM_30m <- rast(paste0(wd$data_p,"DEM_smooth_30m.tif"))
...
TRI <- terra::terrain(DEM_30m, v="TRI")# calc Terrain Ruggedness Index

Slope <- terra::terrain(DEM_30m, v="slope", unit="degrees")#Slope
Slope_sqrt <- Slope^0.5
...
wbt_breach_depressions_least_cost(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/DEM_breach.tif"), dist = 5)
...
wbt_wetness_index(sca=paste0(wd$data_p,"temp_rasts/Accum.tif"), slope=paste0(wd$data_p,"temp_rasts/Slope_30m.tif"), output=paste0(wd$data_p,"temp_rasts/TWI.tif"), verbose_mode = FALSE)
TWI <- rast(paste0(wd$data_p, "temp_rasts/TWI.tif"))
TWI <- ifel(is.na(TWI), max(terra::values(TWI), na.rm=T), TWI)
writeRaster(TWI,paste0(wd$data_p,"temp_rasts/TWI.tif"), overwrite=TRUE)
#TWI <- focal(TWI, w=17, fun='max', na.policy="only", na.rm=T)
...
wbt_stream_power_index(sca=paste0(wd$data_p,"temp_rasts/Accum.tif"),slope=paste0(wd$data_p,"temp_rasts/Slope_30m.tif"),output=paste0(wd$data_p,"temp_rasts/SPI.tif"),exponent = 1)
SPI <- rast(paste0(wd$data_p, "temp_rasts/SPI.tif"))
...
wbt_sediment_transport_index(sca=paste0(wd$data_p,"temp_rasts/Accum_fd8.tif"),slope=paste0(wd$data_p,"temp_rasts/Slope_30m.tif"),output=paste0(wd$data_p,"temp_rasts/STI.tif"))
STI <- rast(paste0(wd$data_p, "temp_rasts/STI.tif"))

######################################################

wbt_d_inf_pointer(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/FlowDirect_inf_30m.tif"))
FlowDirect_inf_30m <- rast(paste0(wd$data_p, "temp_rasts/FlowDirect_inf_30m.tif"))

wbt_d8_pointer(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/FlowDirect_d8_30m.tif"))
FlowDirect_d8_30m <- rast(paste0(wd$data_p, "temp_rasts/FlowDirect_d8_30m.tif"))

######################################################

wbt_extract_streams(flow_accum = paste0(wd$data_p,"temp_rasts/Accum_fd8.tif"), output = paste0(wd$data_p,"temp_rasts/Streams_30m.tif"), threshold =7500)
Streams <- rast(paste0(wd$data_p, "temp_rasts/Streams_30m.tif"))

######################################################

wbt_isobasins(dem = paste0(wd$data_p,"temp_rasts/DEM_breach.tif"), 
              output = paste0(wd$data_p,"temp_rasts/Isobasins_30m.tif"), 
              size = 333333)
Isobasins <- rast(paste0(wd$data_p, "temp_rasts/Isobasins_30m.tif"))
Isobasins <- as.factor(Isobasins)
writeRaster(Isobasins,paste0(wd$data_p,"temp_rasts/Isobasins30km_30m.tif"), overwrite=TRUE)
...
Municipios2000s <- st_read(paste0(wd$data_p, "Municipios2000s_Data.gpkg"))
intersected_polygons <- st_intersection(IsobasinsPoly, Municipios2000s)
IsobasinsPoly = IsobasinsPoly %>% filter(Isobasins_30m %in% unique(intersected_polygons$Isobasins_30m))
st_write(IsobasinsPoly, paste0(wd$data_p, "IsobasinsPoly.gpkg"), driver = "GPKG", overwrite=TRUE, append=FALSE)
rm(intersected_polygons,Municipios2000s)
...
for (i in 129:130){
  ...
  elev <- mask(elev, bp)
  writeRaster(elev,paste0(wd$data_p,"elev.tif"), overwrite=TRUE)
  wbt_flood_order(dem = paste0(wd$data_p,"elev.tif"), output = paste0(wd$data_p,"BasinRasts_30m/flood_order_basin",i,".tif"))
}
raster_files <- list.files(path = paste0(wd$data_p,"BasinRasts_30m/"), pattern = ".tif$", full.names = TRUE)
raster_list <- list()
for (file in raster_files) {
  raster <- rast(file)
  raster_list[[file]] <- raster
...
BP = extract(DEM_30m, IsobasinsPoly, fun=ncell)
IsobasinsPoly$n_cells = BP[,2]
rm(BP)
bp <- terra::rasterize(IsobasinsPoly, DEM_30m, field = "n_cells", background = NA)
...
Streams2 <- terra::rasterize(streams_sf, DEM_30m, values = 1, background = NA)
...
wbt_elevation_above_stream(dem = paste0(wd$data_p,"temp_rasts/DEM_breach.tif"), streams = paste0(wd$data_p,"temp_rasts/Streams_30m.tif"), output = paste0(wd$data_p,"temp_rasts/ElevAboveStreams_30m.tif"))
ElevAboveStreams <- rast(paste0(wd$data_p, "temp_rasts/ElevAboveStreams_30m.tif"))

######################################################

#wbt_elevation_above_stream(dem = paste0(wd$data_p,"temp_rasts/DEM_breach.tif"), streams = paste0(wd$data_p,"Streams2_30m.tif"), output = paste0(wd$data_p,"ElevAboveStreams2_30m.tif"))
#ElevAboveStreams2 <- rast(paste0(wd$data_p, "ElevAboveStreams2_30m.tif"))

######################################################

#wbt_extract_valleys(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), variant = 'JandR', line_thin = F, output =  paste0(wd$data_p,"temp_rasts/valleys_JandR_30m.tif"))
...
IsobasinsPoly <- rownames_to_column(IsobasinsPoly, var = "ID")
#BasinsPoly <- vect(BasinsPoly)
Isobasins <- terra::rasterize(IsobasinsPoly, DEM_30m, field = "ID", background = NA)
writeRaster(Isobasins,paste0(wd$data_p,"Isobasins_30m.tif"), overwrite=TRUE)
Isobasins <- rast(paste0(wd$data_p, "Isobasins_30m.tif"))

######################################################

wbt_elev_relative_to_watershed_min_max(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), watersheds = paste0(wd$data_p,"Isobasins_30m.tif"), output = paste0(wd$data_p,"temp_rasts/elev_watershed_30m.tif"))
elev_watershed <- rast(paste0(wd$data_p, "temp_rasts/elev_watershed_30m.tif"))

wbt_minimal_curvature(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_min_30m.tif"))
curv_min <- rast(paste0(wd$data_p, "temp_rasts/curv_min_30m.tif"))

wbt_maximal_curvature(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_max_30m.tif"))
curv_max <- rast(paste0(wd$data_p, "temp_rasts/curv_max_30m.tif"))

wbt_mean_curvature(dem = paste0(wd$data_p,"DEM_smooth_30m.tif"), output = paste0(wd$data_p,"temp_rasts/curv_mean_30m.tif"))
...
Topo_30m <- c(DEM_30m, Slope, Slope_sqrt, Accum, Accum_fd8, TRI, TWI, STI, SPI, curv_min, curv_max, curv_mean, DistStreams, DistStreams2, ElevAboveStreams, elev_watershed, FloodOrder_Rel)#ElevAboveStreams2, 

names(Topo_30m) = c("DEM", "Slope", "Slope_sqrt", "Accum", "Accum_fd8", "TRI", "TWI", "STI", "SPI", "curv_min", "curv_max", "curv_mean", "DistStreams", "DistStreams2", "ElevAboveStreams", "elev_watershed", "FloodOrder_Rel")#"ElevAboveStreams2", 

Topo_30m <- project(Topo_30m, resample_30m)
Topo_30m <- resample(Topo_30m, resample_30m, method = "bilinear")

writeRaster(Topo_30m, filename = paste0(wd$data_p, "Topo_r_resampled_30m.tif"), overwrite=TRUE)

Topo_30m <- rast(paste0(wd$data_p, "Topo_r_resampled_30m.tif"))

rm(DEM_30m, Slope, Slope_sqrt, Accum, Accum_fd8, TRI, TWI, STI, SPI, curv_min, curv_max, curv_mean, DistStreams, DistStreams2, ElevAboveStreams, Isobasins, IsobasinsPoly, FlowDirect_d8_30m, FlowDirect_inf_30m, elev_watershed, FloodOrder_Rel, FloodOrder, Valleys, FlowDirect_d8_30m, FlowDirect_inf_30m, Streams, Streams2, Basins, streams_sf, DEM_smooth_30m, DEM, Topo_30m)
...
names(BDTICM) <- "BDTICM"
BDTICMi <- RF_ImputeRast(r = BDTICM, covs = c(DEM_250,Slope_250, BDRICM_250), Mask = m, SampFrac = 0.3, save_mem = FALSE)
BDTICM <- BDTICMi[[1]]
BDTICM_250 = BDTICM
...
AWCtSi <- RF_ImputeRast(r = AWCtS, covs = c(DEM_250,Slope_250,BDTICM_250), Mask = m, SampFrac = 0.3, save_mem = FALSE)
AWCtS_2mi <- RF_ImputeRast(r = AWCtS_2m, covs = c(DEM_250,Slope_250,BDTICM_250), Mask = m, SampFrac = 0.3, save_mem = FALSE)
AWCtS <- AWCtSi[[1]]
AWCtS_2m <- AWCtS_2mi[[1]]
AWCtS_250 <- AWCtS
AWCtS_2m_250 <- AWCtS_2m
...
AWCh1i <- RF_ImputeRast(r = AWCh1, covs = c(DEM_250,BDRICM_250, AWCtS_250), Mask = m, SampFrac = 0.3, save_mem = FALSE)
AWCh1_2mi <- RF_ImputeRast(r = AWCh1_2m, covs = c(DEM_250,BDRICM_250, AWCtS_2m_250), Mask = m, SampFrac = 0.3, save_mem = FALSE)
AWCh1 <- AWCh1i[[1]]
AWCh1_2m <- AWCh1_2mi[[1]]
AWCh1_250 <- AWCh1
AWCh1_2m_250 <- AWCh1_2m
AWCh1_temp <- AWCh1
...
BDRICM_igh <- project(BDRICM_250, SGigh)
BDRICM_igh <- resample(BDRICM_igh, SGigh, method = "bilinear")
BDTICM_igh <- project(BDRICM_250, SGigh)
BDTICM_igh <- resample(BDRICM_igh, SGigh, method = "bilinear")
AWCtS_igh <- project(AWCtS_250, SGigh)
AWCtS_igh <- resample(AWCtS_igh, SGigh, method = "bilinear")
AWCtS_2m_igh <- project(AWCtS_2m_250, SGigh)
AWCtS_2m_igh <- resample(AWCtS_2m_igh, SGigh, method = "bilinear")
AWCh1_igh <- project(AWCh1_250, SGigh)
AWCh1_igh <- resample(AWCh1_igh, SGigh, method = "bilinear")
AWCh1_2m_igh <- project(AWCh1_2m_250, SGigh)
AWCh1_2m_igh <- resample(AWCh1_2m_igh, SGigh, method = "bilinear")
...
names(CF) <- "CF"
CFi <- RF_ImputeRast(r = CF, covs = c(DEM_igh, Slope_igh, AWCtS_igh, BDRICM_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
CF <- CFi[[1]]
CF_igh <- CF
...
names(BD) <- "BD"
BDi <- RF_ImputeRast(r = BD, covs = c(DEM_igh, Slope_igh, AWCtS_igh, AWCh1_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
BD <- BDi[[1]]
BD_igh <- BD
...
names(Z) <- "Z"
Zi <- RF_ImputeRast(r = Z, covs = c(DEM_igh, Slope_igh, AWCtS_igh, AWCh1_igh, BD_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
Z <- Zi[[1]]
Z_igh <- Z
...
names(C) <- "C"
Ci <- RF_ImputeRast(r = C, covs = c(DEM_igh, Slope_igh, AWCtS_igh, AWCh1_igh, Z_igh, BDTICM_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
C <- Ci[[1]]
C_igh <- C
...
names(S) <- "S"
Si <- RF_ImputeRast(r = S, covs = c(DEM_igh, C_igh, AWCtS_igh, AWCh1_igh, Z_igh, BDTICM_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
S <- Si[[1]]
S_igh <- S
...
names(SOCS) <- "SOCS"
SOCSi <- RF_ImputeRast(r = SOCS, covs = c(DEM_igh, Slope_igh, AWCtS_igh, AWCh1_igh, BD_igh, Z_igh, C_igh, BDRICM_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
SOCS <- SOCSi[[1]]
SOCS_igh <- SOCS
...
names(SOC) <- "SOC"
SOCi <- RF_ImputeRast(r = SOC, covs = c(DEM_igh, Slope_igh, SOCS_igh, AWCtS_igh, AWCh1_igh, BD_igh, Z_igh, C_igh, CF_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
SOC <- SOCi[[1]]
SOC_igh <- SOC
...
names(OCD) <- "OCD"
OCDi <- RF_ImputeRast(r = OCD, covs = c(DEM_igh, Slope_igh, SOC_igh, AWCtS_igh, AWCh1_igh, BD_igh, Z_igh, C_igh, BDRICM_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
OCD <- OCDi[[1]]
OCD_igh <- OCD
...
names(N) <- "N"
Ni <- RF_ImputeRast(r = N, covs = c(DEM_igh, Slope_igh, AWCtS_igh, BD_igh, Z_igh, CF_igh, OCD_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
N <- Ni[[1]]
...
names(CEC) <- "CEC"
CECi <- RF_ImputeRast(r = CEC, covs = c(DEM_igh, S_igh, BDRICM_igh, BDTICM_igh, C_igh, CF_igh, AWCtS_2m_igh), Mask = m2, SampFrac = 0.3, save_mem = FALSE)
CEC <- CECi[[1]]
...
rm(DepthWeights, m2, m, resample_250m, resample_igh, SGigh, DEM_igh, Slope_igh, DEM_250, Slope_250, SG250, DEM_100m, sgwd, AWCtS_2m_250, AWCtS_250, AWCh1_2m_250, AWCh1_250, BDTICM_250, BDRLOG_250, BDRICM_250, Z_igh, S_igh, CF_igh, SOC_igh, CEC_igh, OCD_igh, N_igh, SOCS_igh, C_igh, BD_igh, AWCtS_2m_igh, AWCtS_igh, AWCh1_igh, AWCh1_2m_igh, BDTICM_igh, BDRLOG_igh, BDRICM_igh)
...
z <- resample(Soil[[1]], resample_30m, method = "bilinear")

for (i in 2:(nlyr(Soil)-1)) {
  a = resample(Soil[[i]], resample_30m, method = "bilinear")
  z = c(z,a)
}
WRB <- resample(Soil[[nlyr(Soil)]], resample_30m, method = "near") #Soil[[13]]
Soil_30m = c(z, WRB)
rm(z,a, WRB)

writeRaster(Soil_30m, filename = paste0(wd$data_p, "Soil_r_resampled_30m.tif"), overwrite=TRUE)
rm(Soil_30m)
...
Clim_30m <- resample(Clim[[1]], resample_30m, method = "bilinear")

for (i in 2:(nlyr(Clim)-1)) {
  if (is.factor(Clim[[i]])){
    a = resample(Clim[[i]], resample_30m, method = "near")
  } else {
    a = resample(Clim[[i]], resample_30m, method = "bilinear")
  }
  Clim_30m = c(Clim_30m,a)
}

writeRaster(Clim_30m, filename = paste0(wd$data_p, "Clim_r_resampled_30m.tif"), overwrite=TRUE)
rm(Clim_30m,a)
