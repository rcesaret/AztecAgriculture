---
title: "Agricultural Productivity Model"
...
packages <- c("tidyverse", "rgdal", "rgeos", "sp", "sf", "GISTools", "raster", 
              ...
              "stringi", "fuzzyjoin", "mgcv", "randomForest", "ranger")#, "data.table", "zoo", "era", "JOPS", "mgcv","igraph",  "ggrepel","ggridges", "movecost",  "datplot", "scales",
...
FUNCS <- list("RescaleSpatRast.R", "Terra_df.R")#"splitByAttributes.R", 
...
```{r, 'Load/Organize SIAP 2003-2021 Yield Polygons', message=FALSE, warning=FALSE}
# Read-in the data
...
###DEM <- rast("D:/Dropbox (ASU)/AztecAgricultureModel/GIS_Data/DEM_CEM_v3.0_15m_CMex.tif")
...
```{r}
# Read the raster data and create a SpatRast object:
r <- rast("your_raster_file.tif")

# Identify the cells with NA values:
na_cells <- is.na(r)

# Extract the coordinates of the NA cells:
coords <- xyFromCell(r, which(na_cells), spatial = TRUE)

# Extract the values of the raster at the non-NA cells:
values <- getValues(r)[!na_cells]

# Fit a GAM model to the non-NA cells:
gam_model <- gam(values ~ s(x, y), data = coords)

# Predict the values for the NA cells using the fitted GAM model:
predicted_values <- predict(gam_model, newdata = coords)

# Assign the predicted values to the NA cells in the original raster:
r[na_cells] <- predicted_values
...
RF_ImputeRast <- function(r = BD, # Terra SpatRast raster object
                          rnam = names(r),
                          covs = c(DEM_r,Slope)#=NULL,
						  Mask = m,
						  SampFrac = 0.3,
						  # character vector name of column name for raster values
                     ) {
  
  m_df = Terra_df(m, "mask")
  ...
  if (!is.null(covs)){
    ...
    #covs_df <- bind_cols(out_list)
    #covs_df <- do.call(cbind, df_list)
    r_df = merge(r_df, covs_df, all = F, sort = F)
  }
  

  train = r_df %>% filter(outside == F) %>% filter(complete.cases(.)) %>% select(-na_cells, -outside, -mask) %>% sample_frac(SampFrac)
...
m_df = Terra_df(m, "mask")
names(m_df) <- c("x", "y", "mask", "outside")
BD_df = Terra_df(BD, "BD")
BDRICM_df = Terra_df(BDRICM, "BDRICM") 
BDTICM_df = Terra_df(BDTICM, "BDTICM")
Elev_df = Terra_df(DEM_r, "Elevation")
Slope_df = Terra_df(Slope, "Slope")

Elev_df2 = Elev_df %>% select(-na_cells)
Slope_df2 = Slope_df %>% select(-na_cells)
BDRICM_df2 = BDRICM_df %>% select(-na_cells)
BDTICM_df2 = BDTICM_df %>% select(-na_cells)
z = BD_df %>% left_join(m_df, by=c('x','y')) %>% left_join(Elev_df2, by=c('x','y')) %>%
  left_join(Slope_df2, by=c('x','y')) #%>%
  #left_join(BDRICM_df2, by=c('x','y')) %>% 
  #left_join(BDTICM_df2, by=c('x','y'))

zt <- z %>% filter(outside == F) %>% filter(complete.cases(.)) %>% select(-na_cells, -outside, -mask)

zt2 <- zt %>% sample_frac(0.3)

#rm(BD_df,BDRICM_df,BDTICM_df,Elev_df,Slope_df)
...
missing_vals <- is.na(r)
predicted_vals <- predict(rf_model, df[, -1])
r[missing_vals] <- predicted_vals
...
##resample_30m <- rast(paste0(wd$data_r, "resample_30m.tif"))
#Soil_r_resampled = rast(Soil_resampled)
#S2 <- resample(rast(S), resample_30m, method = "bilinear")


# Elevation as downsampling Covariate
#elev <- rast("D:/Dropbox (ASU)/Huexotzinco/HuexTlaxAnal2022/GIS_Data/DEM_CEM_v3.0_15m_CMex.tif")
#elev[elev == 0] <- NA
#elev[is.nan(elev)] <- NA
##x <- rast(paste0(wd$data_r, "SoilGrids/BDRICM.tif"))
#elev <- resample(elev, rast(paste0(wd$data_r, "SoilGrids/BDRICM.tif")), method = "bilinear")

#x <- spatialEco::raster.downscale(BDRICM, elev, scatter=TRUE, residuals = TRUE,
...
# 	BDRLOG	Probability of occurrence of R horizon	percentage
...
# Nitrogen
N_0_5 <- rast(paste0(wd$data_r, "SoilGrids/N_0_5.tif"))
N_0_5[N_0_5 == 0] <- NA
...
SOCS[SOCS == 0] <- NA
SOCS <- resample(rast(SOCS), resample_100m, method = "bilinear")
names(SOCS) <- "SOCS"

#Stack
Soil = c(BD, S, Z, C, SOC, CEC, OCD, N, SOCS, BDRICM, BDRLOG, BDTICM)
#names(Soil) = c("BD", "S", "Z", "C", "SOC", "CEC", "OCD", "N", "SOCS", "BDRICM", "BDRLOG", "BDTICM")
...
PH = pH water
WRB = World Reference Base (2006) Soil Groups
...
```{r}
chwd = "D:/Dropbox (ASU)/Huexotzinco/HuexTlaxAnal2022/GIS_Data/CHELSA/"
prj = "+proj=utm +zone=14 +datum=WGS84 +units=m +no_defs"
EXT <- projectRaster(WRB, crs = "+proj=longlat +datum=WGS84 +no_defs")
resample_30m <- rast(paste0(wd$data_r, "resample_30m.tif"))
...
writeRaster(Clim_r, filename = paste0(wd$data_r, "Clim_r_resampled.tif"))
...
```{r}
krige_rasts <- function(raster_missing, raster_cov = NULL){
  
  raster_missing = BDRICM
  raster_cov = elev
  
  df <- data.frame(xyFromCell(raster_missing, 1:ncell(raster_missing)))
  v <- values(raster_missing)
  #i <- !is.na(v)
  df$v <- as.numeric(v)
  cov <- values(raster_cov)
  i <- !is.na(cov) & !is.na(v)
  df$cov <- as.numeric(cov)
  names(df) = c("x","y","missing", "cov")
  dfi <- df[i,]
  
  vu <- variogram(missing~cov, ~x+y, data=dfi)
  ...
  # Create a mask indicating the locations of missing values
  mask <- is.na(raster_missing)
 # mask <- terra::mask(raster_missing, fun = is.na)
  
  # Create a SpatRaster object with the same extent and resolution as the original raster,
  # containing only the missing values
  missing_points <- mask(raster_missing, mask = mask)
  #missing_points <- rast(extent(raster_missing), resolution(raster_missing), nlyrs = 1)
  values(missing_points) <- NA
  
  # Generate a SpatVector object representing the known values in the raster
  known_points <- mask(raster_missing, mask = !mask)
  #known_points <- vect(mask, as.points = TRUE)
  values(known_points) <- values(raster_missing)[mask]
  
  if (is.null(raster_cov)){
    
    kriging_model <- kriging(known_points ~ 1, data = known_points)
    predicted_values <- predict(kriging_model, newdata = missing_points)
    raster_filled <- raster_missing
    raster_filled[mask] <- predicted_values
    
  } else {
    # Combine the known values and elevation as covariates
    covariate_data <- cbind(values(known_points), values(raster_cov)[mask])
    
    # Fit a kriging model using the known values and elevation as covariates
    kriging_model <- kriging(known_points ~ ., data = covariate_data)
    
    # Predict the missing values using the kriging model and covariate data
    covariate_data_missing <- cbind(values(missing_points), values(raster_cov)[!mask])
    predicted_values <- predict(kriging_model, newdata = covariate_data_missing)
    
    # Replace the missing values in the original raster with the predicted values
    raster_filled <- raster_missing
    raster_filled[mask] <- predicted_values
  }
