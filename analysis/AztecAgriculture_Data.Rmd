...
# ONE ISSUE MAY BE FIXED BY SWITCHING THJE ORDER OF LU2000c and LU2000d
# Changing AGType for specific municipalities FIRST, and THEN merging the polygons!!!
...
SIAP_3 = SIAP_2 %>% left_join(SIAP_ID, by = c('Estado_ID', 'Estado', 'ddr_ID', 'ddr', 'Cader_ID', 'Cader', 'Municipio_ID', 'Municipio', 'EstadoMunicipio')) %>% 
  # filter the data to only include Maize stats and municipalities in the CMex focal region
  filter(Cultigen == "Maiz grano", INCLUDE == TRUE) %>%
  # convert units -- tons to kg -- for yield and product
...
SIAP_4 = SIAP_3 %>% 
  ...
  # calculate variables for the maize yield time-series mean, standard deviation 
  ...
  # calculate variables for the mean, standard deviation and coefficient of variation 
  # for each agriculture type by state-municipality
  ...
  mutate(
    # calculate the maize yield Z-score by state-municipality for all agriculture types
    ...
    # the temporal maize yield coefficient of variation for each state-municipality
