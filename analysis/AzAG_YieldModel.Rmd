...
Soil_sub <- subset(Soil, c("BDRICM", "BDTICM", "AWCtS", "AWCtS_2m", "AWCh1", "AWCh1_2m", "S", "Z", "C", "CF", "SOC", "CEC","OCD", "N", "SOCS"))

Topo_sub <- subset(Topo, c("DEM", "Slope", "Slope_sqrt", "Accum", "TWI", "STI", "SPI","DistStreams", "DistStreams2", "ElevAboveStreams", "elev_watershed", "FloodOrder_Rel"))
...
clt_april = subset(Clim, c("clt_04"))
...
rsds_april = subset(Clim, c("rsds_04"))
...
Clim_sub <- c(Clim_sub, pr_april, pr_may, pr_june, pr_spring45, pr_spring56, pr_summer78, pr_summer678, pr_fall, pr_sept, pr_oct, pet_april, pet_may, pet_june, pet_spring45, pet_spring56, pet_summer78, pet_summer678, pet_fall, pet_sept, pet_oct, cmi_april, cmi_may, cmi_spring45, cmi_june, cmi_spring56, cmi_summer78, cmi_summer678, cmi_fall, cmi_sept, cmi_oct, clt_april, clt_may, clt_june, clt_spring56, clt_summer678, clt_summer78, clt_fall, clt_sept, clt_oct, rsds_april, rsds_may, rsds_june, rsds_spring, rsds_summer78, rsds_summer789, rsds_sept, rsds_oct, rsds_fall)
...
st_write(Data2000s_poly_rs, paste0(wd$data_p, "Data2000s_poly_rescale_HighElev.gpkg"), driver = "GPKG", overwrite=T, append=FALSE)

Data2000s_poly_rs <- st_read(paste0(wd$data_p, "Data2000s_poly_rescale_HighElev.gpkg"))
...
st_write(Train_sf_full, paste0(wd$data_p, "Train_sf_full.gpkg"), driver = "GPKG", overwrite=T, append=FALSE)
...
st_write(Train_sf, paste0(wd$data_p, "Train_sf.gpkg"), driver = "GPKG", overwrite=T, append=FALSE)
...
```{r}
Env_sub <- rast(paste0(wd$data_p, "Env_sub_RF.tif"))
Data2000s_poly_rs <- st_read(paste0(wd$data_p, "Data2000s_poly_rescale_HighElev.gpkg")) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))
Train_sf_full <- st_read(paste0(wd$data_p, "Train_sf_full.gpkg")) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))
Train_sf <- st_read(paste0(wd$data_p, "Train_sf.gpkg")) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))
Train_sf_Both_AvgYield = Train_sf %>% select(-cvYield)
Train_sf_Both_cvYield = Train_sf %>% select(-AvgYield)
...
```{r}
Data_sf <- st_centroid(Train_sf_Both_AvgYield) %>% select(-Estado, -Municipio, -EstadoMunicipio) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))

Data_df = st_drop_geometry(Data_sf)

Data_sf2 = Data_sf %>% select(AvgYield, AGType, AvgPctArea_ofTotal, TotalLabor_DensArable, Fertilizer, mean.DEM:mean.rsds_fall)
                                              
Data_df2 = st_drop_geometry(Data_sf2)
```

## Boruta Feature Selection

```{r}
boruta <- Boruta(AvgYield ~ ., data = Data_df2, maxRuns = 500)#, doTrace = 2
...
Data_sf_tsk = as_task_regr_st(Data_sf2, target = "AvgYield", id = "Data_sf")
...
FFrac = c(1.0, 0.95, 0.90, 0.85, 0.975, 0.925, 0.875, 0.825, 0.80, 0.775, 0.75, 0.725, 0.70, 0.675, 0.65, 0.625, 0.60, 0.55, 0.5, 0.45, 0.4, 0.35, 0.3)

names(Data_sf3) = sub("^mean\\.", "", names(Data_sf3))
#Data_sf = Data_sf %>% select(-ElevAboveStreams2)
Data_sf_tsk = as_task_regr_st(Data_sf3, target = "AvgYield", id = "Data_sf3")
...
Data_sf_train = as_task_regr_st(Data_sf3, target = "AvgYield", id = "Data_sf_train")
Data_sf_test = as_task_regr_st(Data_sf3, target = "AvgYield", id = "Data_sf_test")
...
names(pred) <- c("ID", "AvgYield_Actual", "AvgYield_Predict_1.0", "AvgYield_Resid_1.0")
pred_df[[1]] <- pred
names(pred_df)[1] <- "Feat_1.0"
...
writeRaster(AvgYield_Pred, filename = paste0(wd$data_p, "AvgYield_Pred.tif"), overwrite=TRUE)
```
...
rfecv = fselect(
  fselector = fs("rfecv", n_features = FEAT[i], feature_fraction = FFrac[i]),
...
#   rfecv$result
  temp_task = as_task_regr_st(Data_sf3, target = "AvgYield", id = "temp_task")
  temp_task$select(rfecv$result_feature_set)
  temp_train = as_task_regr_st(Data_sf3, target = "AvgYield", id = "temp_train")
  temp_train$select(rfecv$result_feature_set)
  temp_train = temp_train$filter(splits$train)
  temp_test = as_task_regr_st(Data_sf3, target = "AvgYield", id = "temp_test")
  temp_test$select(rfecv$result_feature_set)
  temp_test = temp_test$filter(splits$test)

  
  rr = resample(temp_train, at, resampling = rsmp("cv", folds = 3), store_models = T)
  ...
  b = as.data.frame(rfecv$result) %>% rename(fselect_rsq = regr.rsq)
  c = as.data.frame(rr$learners[[1]]$state$model$tuning_instance$result) %>% rename(tune_rsq = regr.rsq)
    # Train model on full data for predictions
  df = cbind(a,b,c)
  results[[i]] <- df
  ...
  pred = as.data.frame(as.data.table(pred))
  pred$resid = pred$response - pred$truth
  names(pred) <- names(pred_df[[1]]) <- c("ID", "AvgYield_Actual", paste0("AvgYield_Predict_",FFrac[i]), paste0("AvgYield_Resid_",FFrac[i]))
  pred = pred[,-c(1:2)]
  pred_df[[i]] <- pred
...
names(pred_df) = NULL
rfecvFeatSelectSeries_AvgYield_predictions = do.call(cbind, c(pred_df, deparse.level=0))
Data2000s_poly_rs_FSpred = cbind(Data2000s_poly_rs,rfecvFeatSelectSeries_AvgYield_predictions)
st_write(Data2000s_poly_rs_FSpred, paste0(wd$data_p, "Data2000s_poly_rs_HighElev_FSpred.gpkg"), driver = "GPKG", overwrite=T, append=FALSE)
write.csv(st_drop_geometry(Data2000s_poly_rs_FSpred), paste0(wd$data_p,"Data2000s_poly_rs_HighElev_rfecvFS_predictions.csv"))


results[[1]]  = results[[1]] %>% select(!!!syms(names(results[[2]])))
rfecvFeatSelectSeries_AvgYield_results = do.call(rbind, results)
rfecvFeatSelectSeries_AvgYield_results <- rfecvFeatSelectSeries_AvgYield_results %>%
  rowwise() %>% mutate(n_Features = sum(c_across(AGType:swb) == TRUE))
x = rfecvFeatSelectSeries_AvgYield_results %>% ungroup()

x$features <- lapply(x$features, function(x) paste(toString(x), collapse = ", "))
y <- do.call( rbind, x$features)
x$features <- y

x$learner_param_vals <- lapply(x$learner_param_vals, function(x) paste(toString(x), collapse = ", "))
y <- do.call( rbind, x$learner_param_vals)
x$learner_param_vals <- y

x$x_domain <- lapply(x$x_domain, function(x) paste(toString(x), collapse = ", "))
y <- do.call( rbind, x$x_domain)
x$x_domain <- y
write.csv(x, paste0(wd$data_p,"rfecvFeatSelectSeries_AvgYield_results.csv"))


writeRaster(AvgYield_Pred, filename = paste0(wd$data_p, "AvgYield_Pred_FeatSelect.tif"), overwrite=TRUE)
AvgYield_Pred_FeatSelect <- rast(paste0(wd$data_p, "AvgYield_Pred_FeatSelect.tif"))


#rm(AvgYield_Pred, pred_df, results)
...
* 0.5 == 50% of 40 features == 20 features
* R-squared (Generalization) == 0.878
* num.trees = 140
* mtry.ratio = 0.466312354754882
* max.depth = 15
* sample.fraction = 0.927035579649592
...
```{r}
Data_sf <- st_centroid(Train_sf_Both_cvYield) %>% select(-Estado, -Municipio, -EstadoMunicipio) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))

Data_df = st_drop_geometry(Data_sf)

Data_sf2 = Data_sf %>% select(cvYield, AGType, AvgPctArea_ofTotal, TotalLabor_DensArable, Fertilizer, mean.DEM:mean.rsds_fall)
...
```{r}
boruta <- Boruta(cvYield ~ ., data = Data_df2, maxRuns = 500)#, doTrace = 2
boruta_atts <- attStats(boruta)
boruta_atts$Boruta <- scales::rescale(rank(boruta_atts$meanImp)/nrow(boruta_atts))
boruta_atts$feature = row.names(boruta_atts)
FeatSelect = boruta_atts %>% rowid_to_column(var = "ORD") %>% select(ORD, feature, meanImp, decision, Boruta)
...
```{r}
Data_sf3 = Data_sf2 %>% select(cvYield, AGType, AvgPctArea_ofTotal, TotalLabor_DensArable, Fertilizer, mean.DEM, mean.Slope_sqrt, mean.Accum, mean.TWI, mean.STI, mean.DistStreams, mean.ElevAboveStreams, mean.FloodOrder_Rel, mean.BDRICM, mean.BDTICM, mean.AWCtS, mean.S, mean.Z, mean.C, mean.CF, mean.OCD, mean.CEC, mean.N, mean.swb, mean.gdd10, mean.cmi_april, mean.pr_june, mean.cmi_may, mean.cmi_oct, mean.pr_sept, mean.cmi_summer78, mean.rsds_may, mean.pet_oct, mean.pet_summer78, mean.npp, mean.clt_april, mean.rsds_april, mean.pet_june, mean.clt_june, mean.pet_sept, mean.rsds_sept)
#Data_sf3_tsk = as_task_regr_st(Data_sf2, target = "AvgYield", id = "Data_sf_set1")
#Data_df_set1 = st_drop_geometry(Data_sf_set1)
#Data_df_set1_tsk = as_task_regr(Data_df_set1, target = "AvgYield", id = "Data_df_set1")
...
FFrac = c(1.0, 0.975, 0.95, 0.925, 0.90, 0.875, 0.85, 0.825, 0.80, 0.775, 0.75, 0.725, 0.70, 0.675, 0.65, 0.625, 0.60)

names(Data_sf3) = sub("^mean\\.", "", names(Data_sf3))
#Data_sf = Data_sf %>% select(-ElevAboveStreams2)
Data_sf_tsk = as_task_regr_st(Data_sf3, target = "cvYield", id = "Data_sf3")
#nnn = Data_sf_tsk$ncol
FEAT = ceiling(FFrac*length(Data_sf_tsk$feature_names))
...
Data_sf_train = as_task_regr_st(Data_sf3, target = "cvYield", id = "Data_sf_train")
Data_sf_test = as_task_regr_st(Data_sf3, target = "cvYield", id = "Data_sf_test")
Data_sf_train = Data_sf_train$filter(splits$train)
Data_sf_test = Data_sf_test$filter(splits$test)

names(Train_sf_full) = sub("^mean\\.", "", names(Train_sf_full))
Data_sf_Full = st_centroid(Train_sf_full) %>% select(-Estado, -Municipio, -EstadoMunicipio, -cvYield) %>% mutate(AGType = factor(AGType, levels = c("Temporal", "Riego")))
Data_sf_Full_tsk = as_task_regr_st(Data_sf_Full, target = "cvYield", id = "Data_sf_Full")
Data_sf_Full_tsk$select(Data_sf_tsk$feature_names)
...
pred = as.data.frame(as.data.table(pred))
pred$resid = pred$response - pred$truth
names(pred) <- c("ID", "cvYield_Actual", "cvYield_Predict_1.0", "cvYield_Resid_1.0")
pred_df[[1]] <- pred
names(pred_df)[1] <- "Feat_1.0"

# raster / terra predictions
prediction = predict_spatial(Pred_Rast_Modern_Temp, RegRF_trained, format = "terra")
names(prediction) <- "cvYield_Predict_1.0_Temp"
cvYield_Pred <- prediction
prediction = predict_spatial(Pred_Rast_Modern_Irrig, RegRF_trained, format = "terra")
names(prediction) <- "cvYield_Predict_1.0_Irrig"
cvYield_Pred <- c(cvYield_Pred, prediction)

writeRaster(cvYield_Pred, filename = paste0(wd$data_p, "AvgYield_Pred.tif"), overwrite=TRUE)
```
...
#   rfecv$result
  temp_task = as_task_regr_st(Data_sf3, target = "cvYield", id = "temp_task")
  temp_task$select(rfecv$result_feature_set)
  temp_train = as_task_regr_st(Data_sf3, target = "cvYield", id = "temp_train")
  temp_train$select(rfecv$result_feature_set)
  temp_train = temp_train$filter(splits$train)
  temp_test = as_task_regr_st(Data_sf3, target = "cvYield", id = "temp_test")
  temp_test$select(rfecv$result_feature_set)
  temp_test = temp_test$filter(splits$test)
  ...
  pred = as.data.frame(as.data.table(pred))
  pred$resid = pred$response - pred$truth
  names(pred) <- names(pred_df[[1]]) <- c("ID", "cvYield_Actual", paste0("cvYield_Predict_",FFrac[i]), paste0("cvYield_Resid_",FFrac[i]))
  pred = pred[,-c(1:2)]
  pred_df[[i]] <- pred
  names(pred_df)[i] <- paste0("Feat_",FFrac[i])
  ...
  # raster / terra predictions
  prediction = predict_spatial(Pred_Rast_Modern_Temp, RegRF_trained2, format = "terra")
  names(prediction) <- paste0("cvYield_Predict_Temp_",FFrac[i])
  cvYield_Pred <- c(cvYield_Pred, prediction)
  prediction = predict_spatial(Pred_Rast_Modern_Irrig, RegRF_trained2, format = "terra")
  names(prediction) <- paste0("cvYield_Predict_Irrig_",FFrac[i])
  cvYield_Pred <- c(cvYield_Pred, prediction)
  #pred_rast[[i]] <- pred
  #names(pred_rast) <- c(names(pred_rast),paste0("Feat_",FFrac[i]))
