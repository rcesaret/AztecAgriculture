...
Topo_r_30m = c(Topo_r_30m, Streams, Streams2, DistStreams, DistStreams2, ElevAboveStreams, ElevAboveStreams2, Valleys, elev_watershed, FloodOrder_Rel, FloodOrder, FlowDirect_d8_30m, FlowDirect_inf_30m)
names(Topo_r_30m) = c(names(Topo_r_30m), "Streams", "Streams2", "DistStreams", "DistStreams2", "ElevAboveStreams", "ElevAboveStreams2", "Valleys", "elev_watershed", "FloodOrder_Rel", "FloodOrder", "FlowDirect_d8_30m", "FlowDirect_inf_30m")
...
x_df <- x_df[complete.cases(x_df[, -which(names(x_df) == "AGType")]), ]
...
x <- subset(x, c("AGType", "Geomorphons", "DEM", "Slope", "Accum_fd8", "TRI", "TWI", "STI", "SPI", "curv_min", "curv_max", "curv_mean", "Streams", "Streams2", "DistStreams", "DistStreams2", "ElevAboveStreams", "ElevAboveStreams2", "Valleys", "elev_watershed", "FloodOrder_Rel", "FloodOrder", "FlowDirect_d8_30m", "FlowDirect_inf_30m", "BDRICM", "BDRLOG", "BDTICM", "BD", "S", "Z", "C", "SOC", "CEC", "OCD", "N", "SOCS", "WRB"))
...
AGType_RF_df <- AGType_RF_df %>% 
  mutate(Cultiv2 = case_when(
         ...
         AGType == "Irrigation" ~ 2),
    #Problem = FALSE,
         #Cultiv_3100masl = ifelse(AGType == "Irrigation" & DEM >= 3100, TRUE, FALSE),
         #Problem = ifelse(AGType == "Irrigation" & LandUse == "Water", TRUE, Problem),
         #Problem = ifelse(AGType == "Irrigation" & LandUse == "Wetland", TRUE, Problem),
         #Problem = ifelse(AGType == "Irrigation" & LandUse == "Settlement", TRUE, Problem),
         #Problem_3100masl = ifelse(AGType == "Irrigation" & DEM >= 3100, TRUE, Problem),
         ...
         #AGType = ifelse(AGType == "Cultivated" & DEM >= 3100, "Temporal", AGType),

         AGType = factor(AGType2, levels = c(1, 2), labels = c("Temporal", "Irrigation"))) %>%
  select(-AGType2) %>% 
  mutate(Temporal1_wt = 0,
         Temporal1_wt = ifelse(AGType == "Temporal" & DEM >= 3100, (Temporal1_wt+1), Temporal1_wt),
         Temporal1_wt = ifelse(AGType == "Temporal" & LandUseArchy == "Water", (Temporal1_wt+1), Temporal1_wt),
         Temporal1_wt = ifelse(AGType == "Temporal" & LandUseArchy == "Wetland", (Temporal1_wt+1), Temporal1_wt),
         Temporal1_wt = ifelse(AGType == "Temporal" & LandUseArchy == "Settlement", (Temporal1_wt+1), Temporal1_wt),
         Temporal1_wt = ifelse(AGType == "Temporal" & LandUseArchy == "Glacial", (Temporal1_wt+1), Temporal1_wt),
         Temporal1 = ifelse(Temporal1_wt > 0, TRUE, FALSE),
         ...
         Temporal2_wt = 0,
         Temporal2_wt = ifelse(AGType == "Temporal" & DEM >= 2850 & DEM < 3100, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & Slope > 15, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & Accum_fd8 > xxx, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & STI > 15, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & TRI > 15, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & SPI > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & TWI > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & flood_order > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & valleys_lq > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & elev_watershed > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & curv_vert_excess > XXX, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & curv_horiz_excess > XXX, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_min > 0.0024170212, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_min < -0.002, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_max > 0.0015, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_max < -0.0015, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_mean > 0.001, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & curv_mean < -0.001, (Temporal2_wt+1), Temporal2_wt),
         
         Temporal2_wt = ifelse(AGType == "Temporal" & BDRLOG > 70, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & BDRICM < 62.87024, (Temporal2_wt+1), Temporal2_wt),
         #Temporal2_wt = ifelse(AGType == "Temporal" & BDTICM <= 3.36, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & SOC > 350, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & BD < 108, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & OCD > 350, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & N > 300, (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & SOCS > 80, (Temporal2_wt+1), Temporal2_wt),
         
         
         Temporal2_wt = ifelse(AGType == "Temporal" & Geomorphons == "Summit", (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & Geomorphons == "Depression", (Temporal2_wt+1), Temporal2_wt),
         Temporal2_wt = ifelse(AGType == "Temporal" & Geomorphons == "Valley", (Temporal2_wt+1), Temporal2_wt),
         Temporal2 = ifelse(Temporal2_wt > 0, TRUE, FALSE),
         Temporal2 = ifelse(Temporal1 == T & Temporal2 == T & Temporal2_wt <= Temporal1_wt, F, Temporal2),
         Temporal1 = ifelse(Temporal1 == T & Temporal2 == T & Temporal2_wt > Temporal1_wt, F, Temporal1),
         #|
         Irrigation = ifelse(AGType == "Irrigation", TRUE, FALSE),
         Irrigation_wt = ifelse(AGType == "Irrigation", 1, 0),
         
         wt = Temporal1_wt + Temporal2_wt + Irrigation_wt,

         Stratum = case_when(
                Arable == TRUE ~ "IrrigationSample",
                Inarable1 == TRUE ~ "TemporalSample1",
                Inarable2 == TRUE ~ "TemporalSample2",
                .default = "Unsampled"),
         SampSize = case_when(
                Stratum == "IrrigationSample" ~ 225000,
                Stratum == "TemporalSample1" ~ 50000,
                Stratum == "TemporalSample2" ~ 125000,
                .default = 0))


dt = setDT(AGType_RF_df)

a <- dt[Stratum == "IrrigationSample"][sample(.N, 225000, replace = FALSE)]

i1 <- dt[Stratum == "TemporalSample1"][sample(.N, 50000, replace = FALSE, prob = wt)]

i2 <- dt[Stratum == "TemporalSample2"][sample(.N, 125000, replace = FALSE, prob = wt)]
...
rm(z, fgeo, fgeo_col_pc, fgeo_row_pc, fWRB_row_pc, fWRB_col_pc, fWRB, Geomorphons)
...
boruta <- Boruta(Cultiv ~ ., data = AGType_RF_Train_df[,c(2:23)], maxRuns = 100)#, doTrace = 2
print(boruta)
...
AGType <- x[[1]]
AGType <- classify(AGType, matrix(c(-0.5, 0.5, 1,  0.5, 1.5, 2), ncol=3, byrow=TRUE))
AGType <- as.factor(AGType)
levels(AGType) = levels(AGType)[[1]] %>% select(-SIAP_LU_Fields) %>%
...
writeRaster(AGType, filename = paste0(wd$data_p, "AGType.tif"), overwrite=TRUE)
rm(AGType)
AGType <- rast(paste0(wd$data_p,"AGType.tif"))
...
rm(AGType_RF_Train_df, AGType_RF_df, AGType_RF_model, x, form, i, l, parts)
